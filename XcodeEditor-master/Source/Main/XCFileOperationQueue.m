...
- (id)initWithBaseDirectory:(NSString*)baseDirectory
{
    ...
    if (self)
    {
        ...
        _bundlesToCopy = [[NSMutableDictionary alloc] init];
...
- (void)queueBundleWithFilePath:(NSString*)filePath inDirectory:(NSString*)directory
{
    ...
    [[_baseDirectory stringByAppendingPathComponent:directory] stringByAppendingPathComponent:[filePath lastPathComponent]];
    NSURL* destinationUrl = [NSURL fileURLWithPath:destinationPath isDirectory:YES];
    [_bundlesToCopy setObject:sourceUrl forKey:destinationUrl];
}
...
- (void)performCopyBundles
{
    [_bundlesToCopy enumerateKeysAndObjectsUsingBlock:^(NSURL* destinationUrl, NSURL* frameworkPath, BOOL* stop)
     {
    ...
    [_bundlesToCopy removeAllObjects];
}
