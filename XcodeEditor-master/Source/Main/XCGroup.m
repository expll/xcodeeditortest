...
- (void)addBundle:(XCBundleDefinition*)bundleDefinition toTargets:(NSArray*)targets type:(XcodeSourceFileType)type;
{
    NSLog(@"------------------");
    [self addBundle:bundleDefinition type:type];
    XCSourceFile* bundleSourceRef = (XCSourceFile*) [self memberWithDisplayName:[bundleDefinition name]];
    NSLog(@"===============bundleSourceRef==================:%@", bundleSourceRef);
    [self addSourceFile:bundleSourceRef toTargets:targets];
}

- (void)addBundle:(XCBundleDefinition *)bundleDefinition type:(XcodeSourceFileType)type
{
    if (([self memberWithDisplayName:[bundleDefinition name]]) == nil)
    {
        NSDictionary* fileReference;
        if ([bundleDefinition copyToDestination])
        {
            fileReference = [self makeFileReferenceWithPath:[bundleDefinition name] name:nil type:type];
            BOOL copyBundle = NO;
            if ([bundleDefinition fileOperationType] == XCFileOperationTypeOverwrite)
            {
                copyBundle = YES;
            }
            else if ([bundleDefinition fileOperationType] == XCFileOperationTypeAcceptExisting)
            {
                NSString* bundleName = [[bundleDefinition filePath] lastPathComponent];
            ...
            NSLog(@"copyBundle: %d", copyBundle);
            if (copyBundle)
            {
                [_fileOperationQueue queueBundleWithFilePath:[bundleDefinition filePath]
                                                    inDirectory:[self pathRelativeToProjectRoot]];
            }
        }
        else
        {
            NSString* path = [bundleDefinition filePath];
            NSString* name = [bundleDefinition name];
            fileReference = [self makeFileReferenceWithPath:path name:name type:type];
        }
        NSString* frameworkKey = [[XCKeyBuilder forItemNamed:[bundleDefinition name]] build];
        [[_project objects] setObject:fileReference forKey:frameworkKey];
        [self addMemberWithKey:frameworkKey];
    }
    [[_project objects] setObject:[self asDictionary] forKey:_key];
...
- (NSDictionary*)makeFileReferenceWithPath:(NSString*)path name:(NSString*)name type:(XcodeSourceFileType)type
{
    ...
    NSLog(@"NSStringFromXCSourceFileType(type):%@, %d", NSStringFromXCSourceFileType(type), type);
